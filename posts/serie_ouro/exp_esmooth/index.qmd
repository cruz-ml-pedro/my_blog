...
::: justify
Nessa segunda post sobre a cotação do ouro, vou criar o primeiro modelo para realizar previsão dos valores de interesse. Os dados utilizados, assim como a escolha dos parâmetros utilizados para criar os modelos a seguir foram, respectivamente, adiquridos e escolhidos na [parte um](https://pedroom-blog.netlify.app/posts/serie_ouro/eda/) e [dois]() dessa série de postos.

Nesta terceira postagem sobre a cotação do ouro será empregada a mesma estratégia de validação cruzada comparação dos modelos. Assim, os procedimentos e funções adotados nestas etapas não serão novamentes descritos.
:::
...
A identificação do tipo de sazonalidade, tendência e erro em modelos ETS (Erro, Tendência e Sazonalidade) envolve uma análise cuidadosa dos dados da série temporal e a escolha de uma combinação adequada desses componentes com base em evidências estatísticas e conhecimento do domínio. Vou explicar como você pode identificar cada um desses componentes e os tipos possíveis em um modelo ETS:

Tipo de Sazonalidade:

Aditivo: A sazonalidade é aditiva quando as variações sazonais têm a mesma magnitude ao longo do tempo, independentemente do nível da série temporal. Isso significa que as oscilações sazonais são adicionadas ao nível da série temporal. Multiplicativo: A sazonalidade é multiplicativa quando as variações sazonais aumentam ou diminuem em relação ao nível da série temporal. Isso significa que as oscilações sazonais são multiplicadas pelo nível da série temporal. Para identificar o tipo de sazonalidade, você pode plotar a série temporal e observar se as variações sazonais parecem aditivas ou multiplicativas. Se as variações parecerem constantes em termos de amplitude em toda a série, é provável que seja um componente aditivo. Se as variações sazonais parecerem aumentar ou diminuir em relação ao nível da série, é provável que seja um componente multiplicativo.

Tipo de Tendência:

Aditivo: A tendência é aditiva quando a magnitude da mudança na série temporal é constante ao longo do tempo. Isso significa que a tendência é uma adição constante ao nível da série. Multiplicativo: A tendência é multiplicativa quando a magnitude da mudança na série temporal aumenta ou diminui ao longo do tempo em relação ao nível da série. Isso significa que a tendência é uma multiplicação constante pelo nível da série. Para identificar o tipo de tendência, você pode plotar a série temporal e observar se as mudanças na série parecem constantes ou se aumentam/diminuem em relação ao nível da série.

Tipo de Erro:

Erro Aditivo: Os erros são aditivos quando a magnitude dos erros é constante ao longo do tempo. Erro Multiplicativo: Os erros são multiplicativos quando a magnitude dos erros aumenta ou diminui em relação ao nível da série. A identificação do tipo de erro geralmente envolve a análise dos resíduos de um modelo ETS ajustado. Você pode ajustar um modelo ETS aos seus dados e examinar os resíduos para verificar se eles exibem uma variação constante ou uma variação que aumenta ou diminui com o tempo.

É importante notar que, em muitos casos, a identificação do tipo de componentes (sazonalidade, tendência e erro) é uma combinação de análise visual, conhecimento do domínio e testes estatísticos. Você pode ajustar vários modelos ETS com diferentes configurações de componentes e selecionar o modelo que melhor se ajusta aos seus dados com base em critérios como o critério de informação de Akaike (AIC) ou o critério de informação bayesiano (BIC).
:::
...
Assim como o método STL, [explicado anteriormente](https://pedroom-blog.netlify.app/posts/serie_ouro/eda/). Em resumo, a principal diferença entre os modelos STL e ETS está na abordagem de decomposição e na ênfase em previsões. O STL é mais flexível em relação à sazonalidade variável, enquanto o ETS assume uma sazonalidade constante e é mais voltado para previsões.
:::
